...
auto find_digit_pos_part1(std::string_view base, bool last) -> digit_pos
{
    ...
    if (auto pos = last ? base.find_last_of(digits) : base.find_first_of(digits); pos != npos)
    {
...
{
    return find_digit_pos_part1(line, false).digit * 10 + find_digit_pos_part1(line, true).digit;
}

auto find_digit_pos_part2(std::string_view base, bool last) -> digit_pos
{
    static constexpr std::array<digit_string, 9> digit_strings = {digit_string{"one"sv, 1},
                                                                  {"two"sv, 2},
                                                                  {"three"sv, 3},
                                                                  {"four"sv, 4},
                                                                  {"five"sv, 5},
    ...
    for (auto f : digit_strings)
    {
        ...
        {
            if ((last && pos > digit.pos) || (!last && pos < digit.pos) || (digit.pos == npos))
            {
...
{
    return find_digit_pos_part2(line, false).digit * 10 + find_digit_pos_part2(line, true).digit;
}
} // namespace
